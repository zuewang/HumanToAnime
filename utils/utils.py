...
from utils.data import ImageFilelist, ImageFolder
import torch
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        return {'image': F.to_tensor(image), 'label': torch.from_numpy(np.asarray(label, dtype=np.float32))}
...
def get_all_data_loaders(conf):
    ...
    if 'data_root' in conf:
        train_loader_a, test_loader_a = get_data_loader_folder(os.path.join(conf['data_root'], 'anime'), batch_size,
                                              new_size_a, height, width, num_workers, test_data_ratio, crop=True)
        train_loader_b, test_loader_b = get_data_loader_folder(os.path.join(conf['data_root'], 'human'), batch_size,
                                              new_size_b, height, width, num_workers, test_data_ratio, crop=True)
    else:
        print('Please set data_root in configs/<>.yaml')
        exit(1)

    print('Anime: train batches: {}, test: {}'.format(len(train_loader_a), len(test_loader_a)))
    print('Human: train batches: {}, test: {}'.format(len(train_loader_b), len(test_loader_b)))
...
def get_data_loader_folder(input_folder, batch_size, new_size=None,
    ...
    dataset_train = ImageFolder(input_folder, transform=transform_train, return_labels=return_labels)
    dataset_test = ImageFolder(input_folder, transform=transform_test, return_labels=return_labels)

    num_train = len(dataset_train)
    indices = list(range(num_train))
    split = int(np.floor(test_data_ratio * num_train))
    np.random.seed(0)
    np.random.shuffle(indices)
    train_idx, test_idx = indices[split:], indices[:split]
    train_sampler, test_sampler = SubsetRandomSampler(train_idx), SubsetRandomSampler(test_idx)

    train_loader = DataLoader(dataset=dataset_train, batch_size=batch_size, drop_last=True, sampler=train_sampler, num_workers=num_workers) # pin_memory=True make data transfer to gpu faster?
    test_loader = DataLoader(dataset=dataset_test, batch_size=batch_size, drop_last=True, sampler=test_sampler, num_workers=num_workers)
...
def get_scheduler(optimizer, hyperparameters, iterations=-1, pose=False):
    if 'lr_policy' not in hyperparameters or hyperparameters['lr_policy'] == 'constant':
        scheduler = None # constant scheduler
    elif hyperparameters['lr_policy'] == 'step':
        if pose:
            scheduler = lr_scheduler.StepLR(optimizer, step_size=hyperparameters['step_size_pose'],
...
def display_points_on_images(images, keypoints):
    out_images = []
    radius = 3
    # print('images shape: {}, keypoints shape: {}'.format(images.size(), keypoints.size()))
    # print('keypoints:', keypoints[0])
    for i, (img, coords) in enumerate(zip(images, keypoints)):
        np_img = img.cpu().numpy()
        for coord in coords:
            # red points
            x1 = max(0, int(coord[0]) - radius)
            x2 = min(np_img.shape[1], int(coord[0]) + radius)
            y1 = max(0, int(coord[1]) - radius)
            y2 = min(np_img.shape[2], int(coord[1]) + radius)
            # cv2.circle(np_img, (int(coords[j]), int(coords[j+1])), 5, (0, 0, 255))
            np_img[0, y1:y2, x1:x2] = 0.
            np_img[1, y1:y2, x1:x2] = 1.
            np_img[2, y1:y2, x1:x2] = 0.
        out_images.append(torch.from_numpy(np_img).unsqueeze(0))

    return torch.cat(out_images).cuda()
